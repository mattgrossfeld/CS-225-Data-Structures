                            ...
                            queue<TreeNode*>& mergeQueue)
{
	...
	if (singleQueue.empty() && mergeQueue.empty()) //If both are empty.
		return NULL;
	else if (singleQueue.empty() && !mergeQueue.empty()) //If merge queue is not empty
	{
		smallestNode = mergeQueue.front();
		mergeQueue.pop();
		return smallestNode;
	}
	else if (!singleQueue.empty() && mergeQueue.empty()) //If singleQueue is not empty.
	{
	...
	else if (!singleQueue.empty() && !mergeQueue.empty()) //Both not empty
	{
		smallestNode = singleQueue.front();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
	...
	int queueSum = singleQueue.size() + mergeQueue.size();
	while (queueSum != 1)
	{
		TreeNode * smallestNode = removeSmallest(singleQueue, mergeQueue);
		TreeNode * nextSmallest = removeSmallest(singleQueue, mergeQueue);
		TreeNode * parentNode = new TreeNode(smallestNode->freq.getFrequency() + nextSmallest->freq.getFrequency());
		parentNode->right = nextSmallest;
		parentNode->left = smallestNode;
		mergeQueue.push(parentNode);
		queueSum = singleQueue.size() + mergeQueue.size();
	}
