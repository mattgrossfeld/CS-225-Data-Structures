...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
     ...
     */
	if (word.empty() && !test.empty())
		return false;
	else if (!word.empty() && test.empty())
		return false;
	else if (word.empty() && test.empty()) //Empty is an anagram of empty.
		return true;
	if (word.length() != test.length()) //Must be same amount of total chars.
		return false;
	
	Dict<char, int> wordTable(26), testTable(26); //Size 26 since the alphabet is 26 chars.
	...
	typename Dict<char, int>::iterator wordIt, testIt = testTable.begin();
	
	for (wordIt = wordTable.begin(); wordIt != wordTable.end(); wordIt++)
	{
		if (wordIt->second != testIt->second)
			return false;
		testIt++;
	}
