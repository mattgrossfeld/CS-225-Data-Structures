...
class KDTree
{
  ...
  private:
     ...
     */
	/*Vector of points to make it easier for constructor*/

	/* Find the euclidean distance between two points and return it*/
	double eucDistance(Point<Dim> first, Point<Dim> second) const;

	/*Helper function for KDTree constructor. Recursive definition*/
	typename KDTree<Dim>::KDTreeNode* constructorHelper(int first, int last, int dimension, vector<Point<Dim>> points);

	/*Quickselect function*/
	void quickSelect(int first, int last, int dimension, int m, vector<Point<Dim>> points);

	/*Partitioning helper function for quickSelect*/
	int partition(int first, int last, int dimension, int p, vector<Point<Dim>> points);
	
	KDTree<Dim> copy(KDTreeNode* root);
	void clear(KDTreeNode* root);

	vector<Point<Dim>> pointz;

    int finderHelper(const Point<Dim>& query, int dimension, int left, int right) const;
