...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
	...
	transform(word1.begin(), word1.end(), first.begin(), ::toupper);
	transform(word2.begin(), word2.end(), second.begin(), ::toupper);
	
	if (dict.count(first) == 0 || dict.count(second) == 0) 
		return false;

	vector<string> firstVector = dict.at(first), secondVector = dict.at(second);
