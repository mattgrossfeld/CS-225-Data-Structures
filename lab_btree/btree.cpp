...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
	...
	if(first_larger_idx < subroot->elements.size() && subroot->elements.empty() == false)
	{
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
	...
	if (subroot == NULL)
		return;
	if (subroot->elements.size() > first_larger_idx && subroot->elements.empty() == false) 
	{
		if (pair == subroot->elements[first_larger_idx])
			return;
	}

	if (subroot->is_leaf == true)
		subroot->elements.insert(first_larger_idx + subroot->elements.begin(), pair);
	else
	{
		BTreeNode* childNode;
		childNode = subroot->children[first_larger_idx];

		insert(childNode, pair);
		if(order < childNode->elements.size() || childNode->elements.size() == order) 				split_child(subroot, first_larger_idx);
	}
