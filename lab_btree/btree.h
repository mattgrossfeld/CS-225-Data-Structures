...
size_t idxInsertHelper(const std::vector<T>& elements, const C& val, size_t start, size_t end)
{

	  if (elements[start] < val && elements[end] > val && start == end - 1) 
		return end;

	size_t median = (start+end)/2;
	
	if (val < elements[median])
		return idxInsertHelper(elements, val, start, median);
	else if (val > elements[median])
		return idxInsertHelper(elements, val, median, end);
	else if (val == elements[median])
		return median;
	return 0;
}
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
	...
	return idxInsertHelper(elements, val, start, end);
}
